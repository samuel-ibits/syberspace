...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" ref={sectionRef} className="py-24 relative overflow-hidden" style={{ background: "var(--bg-base)" }}>
      <motion.div
      ...
      <div className="space-y-5 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div style={{ x: x1 }} className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {testimonials.slice(0, 2).map((t, i) => <Card key={t.name} t={t} delay={i * 0.1} />)}
        </motion.div>
        <motion.div style={{ x: x2 }} className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {testimonials.slice(2).map((t, i) => <Card key={t.name} t={t} delay={i * 0.1 + 0.2} />)}
        </motion.div>
      </div>
    </section>
  );
    ...
    >
      <div className="flex gap-1 mb-4">
        {Array.from({ length: t.stars }).map((_, i) => (
          <motion.svg key={i} initial={{ scale: 0, rotate: -30 }} whileInView={{ scale: 1, rotate: 0 }} viewport={{ once: true }} transition={{ delay: delay + i * 0.07, type: "spring", stiffness: 260 }}
            className="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </motion.svg>
        ))}
      </div>
      <p className="leading-relaxed mb-6 italic text-sm" style={{ color: "var(--text-muted)" }}>"{t.text}"</p>
      <div className="flex items-center gap-3">
        <motion.div whileHover={{ scale: 1.1 }} className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0" style={{ background: "linear-gradient(135deg, #06b6d4, #7c3aed)" }}>
          {t.avatar}
        </motion.div>
        <div>
