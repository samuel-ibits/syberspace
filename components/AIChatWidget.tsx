...
export default function AIChatWidget({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  function handleFlow(msg: string): boolean {
    ...
    if (flow === "human_issue") {
      ...
      addBot(
        `✅ **Got it!**\n\n👤 **Name:** ${name}\n📬 **Contact:** ${contact}\n💬 **Issue:** ${t}\n\nA team member will reach out to you **within a few hours**.\n\nFor urgent matters: 📞 **+234 815 151 9625** or 📧 **support@syberspace.com.ng**`,
        QUICK_REPLIES_DEFAULT
      ); return true;
    }
    return false;
  ...
  const streamAI = useCallback(async (history: Message[], userMsg: string) => {
    ...
    try {
      ...
      setMessages(p => {
        const copy = [...p];
        copy[copy.length - 1] = { role: "assistant", text: "Sorry, I hit a snag. Please try again or reach us directly at **support@syberspace.com.ng** or **+234 815 151 9625**." };
        return copy;
      });
